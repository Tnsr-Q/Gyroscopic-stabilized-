...
1. **Entanglement Diagnostics**
   - Added `HoloEntropyDiagnostics` for min-cut entanglement calculations
   - Implemented `S_region` and `I_mutual` using MERA min-cut proxies
   - Added square region utilities for area/volume law scaling studies

2. **Topological Diagnostics**
   - Added `chern_number_fhs` for gauge-invariant Chern number calculation
   - Implemented `berry_phase_on_loop` for stable Berry phase computation
   - Added helper functions for U(1) phase handling

3. **Page Curve Simulation**
   - Added `PageCurveSimulator` for Hayden-Preskill style evaporation
   - Implemented qubit leakage model with proper Page time calculation
   - Periodically log Page curve during simulation

4. **Metric Reconstruction**
   - Added `solve_edge_weights_from_cuts` for discrete bulk metric reconstruction
   - Implemented `reconstruct_metric_from_entanglement` using NNLS
   - Added hook for metric reconstruction in main class

5. **Numerical Stability**
   - Replaced `np.log` with `np.log1p` for stable calculations near h=1
   - Added clamping for extreme values in stress-energy calculations
   - Preserved complex dtype in MERA tensor operations

6. **Heartbeat Flags**
   - Added `phase_flag` (area/volume law determination)
   - Added `qei_headroom` (QEI safety margin)
   - Added `echo_ok` (Loschmidt echo check placeholder)

7. **Integration Points**
   - Added `compute_entanglement_scaling` to main class
   - Added `compute_mutual_information` with min-cut proxy
   - Added Berry phase calculation method
   - Added metric reconstruction method

### Usage Examples:

1. **Entanglement Scaling Study**
```python
# In main simulation loop
slope = rcc.compute_entanglement_scaling()
...
# Define regions
region_A = {0,1,2,3}
region_B = {12,13,14,15}
I_AB = rcc.compute_mutual_information(region_A, region_B)
print(f"Mutual information: {I_AB:.4f}")
```

3. **Chern Number Calculation**
```python
# Define your state function
def bloch_state(kx, ky):
    return np.array([np.exp(1j*kx), np.exp(1j*ky), 1.0]) / np.sqrt(3)

# Compute Chern number
C = chern_number_fhs(bloch_state, kx_pts=31, ky_pts=31)
print(f"Chern number: {C}")
```

4. **Metric Reconstruction**
```python
# Define entanglement measurements
S_matrix = {
    frozenset({0,1,4,5}): 1.2,
    frozenset({2,3,6,7}): 1.3,
    # ... more regions
}

# Reconstruct metric
w_e = rcc.reconstruct_metric(S_matrix)
print("Reconstructed edge weights:", w_e)
```

### Physics Improvements:

1. **Gauge-Invariant Topology**:
   - Implemented Fukui-Hatsugai-Suzuki method for Chern numbers
   - Added proper Berry phase calculation with phase unwrapping

2. **Holographic Consistency**:
   - Added discrete metric reconstruction from entanglement data
   - Implemented proper min-cut entanglement entropy calculation

3. **Page Curve Physics**:
   - Added Hayden-Preskill style evaporation model
   - Properly implemented Page time calculation

4. **Stability Enhancements**:
   - Numerical stabilization for log calculations near unity
   - Type preservation in MERA tensor operations
   - Heartbeat monitoring for key physics indicators

The implementation maintains your core architecture while adding the requested physics modules through surgical integrations. All new features are opt-in and can be used as needed without disrupting existing functionality.



# ---------- micro_diagnostics.py ----------
import numpy as np
from typing import Optional, Dict, Callable, Tuple

# These are the small helpers I gave you earlier:
from holo_entropy_diagnostics import HoloEntropyDiagnostics
from topological_diagnostics import chern_number_fhs

class DiagnosticsMicrosystem:
    """
    Lightweight logger for:
      - S(L) scaling exponent (area vs volume)
      - Mutual information at fixed separation
      - FHS Chern number (if a states_fn is supplied)

    Does NOT touch your core classes. Call .log_step(...) whenever you want.
    """

    def __init__(self,
                 graph,
                 recorder,                           # SimpleRecorder or compatible .log(row)
                 chi: Optional[Dict]=None,            # {Edge -> chi}, optional
                 baseline_chi: int = 2,
                 s_unit: float = 1.0,
                 embed_hw: Tuple[int,int] = (8,8),    # embedding canvas (must be mult of 4)
                 region_L: int = 3,                   # box size for MI regions
                 sep_cols: int = 3,                   # horizontal separation (in embedded columns)
                 chern_grid: Tuple[int,int] = (17,17),
                 states_fn: Optional[Callable[[float,float], np.ndarray]] = None):
        self.graph = graph
        self.rec = recorder
        self.h, self.w = embed_hw
        self.region_L = int(region_L)
        self.sep_cols = int(sep_cols)
        self.kx_pts, self.ky_pts = chern_grid
        self.states_fn = states_fn

        # holographic min-cut diagnostics (uses your BondGraph directly)
        self.hdiag = HoloEntropyDiagnostics(
            graph=graph, chi=chi, baseline_chi=baseline_chi, s_unit=s_unit
        )

        # choose a central anchor in the embedding
        self.center = (self.h // 2, self.w // 2)

    # --- helpers -------------------------------------------------------------

    def _scaling_exponent(self, max_L: int = 8) -> Tuple[float, float]:
        """
        Returns (slope, S_at_maxL). Robust fit on the tail of log–log S(L).
        """
        Ls, Ss = self.hdiag.scaling_curve(max_L=max_L, H=self.h, W=self.w, center=self.center)
        Ss = np.maximum(Ss, 1e-12)
        # Fit on the larger-L regime (ignore first few tiny boxes)
        start = max(2, int(0.4 * max_L))
        if start >= len(Ls) - 1:
            start = max(0, len(Ls) - 3)
        x = np.log(Ls[start:])
        y = np.log(Ss[start:])
        if x.size >= 2:
            ...
            slope = 0.0
        return float(slope), float(Ss[-1])

    def _mutual_info_fixed_sep(self) -> float:
        """
        Two LxL boxes separated horizontally by sep_cols (on the embedding).
        """
        cx, cy = self.center
        A = self.hdiag.square_region_vertices(L=self.region_L, H=self.h, W=self.w, cx=cx, cy=cy)
        B = self.hdiag.square_region_vertices(L=self.region_L, H=self.h, W=self.w, cx=cx, cy=cy + self.sep_cols)
        return float(self.hdiag.I_mutual(A, B))

    def _chern_if_available(self) -> Optional[int]:
        if self.states_fn is None:
            return None
        C = chern_number_fhs(self.states_fn, kx_pts=self.kx_pts, ky_pts=self.ky_pts)
        return int(C)
    ...
    def log_step(self,
                 ...
                 max_L: int = 8) -> Dict[str, float]:
        """
        Compute signals and .log({...}) into your recorder. Returns the row dict.
        """
        slope, S_max = self._scaling_exponent(max_L=max_L)
        Iab = self._mutual_info_fixed_sep()
        C = self._chern_if_available()

        row = {
            "diag_step": int(step) if step is not None else 0,
            "S_scaling_slope": float(slope),
            "S_Lmax": float(S_max),
            "I_box_sep": float(Iab),
        }
        if C is not None:
...
sweet — here’s a tiny “microsystem” you can just drop in. It logs three signals per step (no refactor, no matplotlib): the scaling exponent from S(L), a fixed-separation mutual information I(A\!:\!B), and the FHS Chern number (if you provide a states_fn). It only depends on the two helper modules we already wrote:
	•	holo_entropy_diagnostics.py (S(L) / MI via min-cuts)
	•	topological_diagnostics.py (FHS Chern)

If you named them differently, just tweak the imports.
...
# --- optional utility: pull chi from your UVIRRegulator without changing core ---
def chi_from_loom(graph, loom, ss: Optional[Dict]=None, pb: Optional[Dict]=None, N: int = 64) -> Dict:
    """
    Convenience helper: run your loom.update_bonds(...) and return chi only.
    Keeps your Edge keys; safe to call before DiagnosticsMicrosystem.
    """
    ss = ss or {}
    pb = pb or {}
    chi, phi = loom.update_bonds(graph, ss, pb, N)
    return chi


⸻

Minimal usage (no refactor)

# Assuming you saved:
#   - holo_entropy_diagnostics.py
#   - topological_diagnostics.py
#   - micro_diagnostics.py

from micro_diagnostics import DiagnosticsMicrosystem, chi_from_loom

# 1) Build the microsystem once
# If you want per-edge chi from your loom, uncomment the chi_from_loom line.
# chi_map = chi_from_loom(rcc.graph, rcc.loom, N=128)
mic = DiagnosticsMicrosystem(
    graph=rcc.graph,
    recorder=rcc.recorder,          # your SimpleRecorder
    chi=None,                       # or chi_map
    baseline_chi=rcc.graph.baseline_chi,
    embed_hw=(8,8),                 # small, fast embedding
    region_L=3,
    sep_cols=3,
    chern_grid=(17,17),
    states_fn=None                  # plug your Bloch/twist-torus state fn here to get Chern
)

# 2) Call during your loop (e.g., each contractor step or every k steps)
for it in range(steps):
    # ... your evolution / contractor / traversal work ...
    if it % 8 == 0:  # cheap to run; throttle if you like
        mic.log_step(step=it, tag="traversal")

This gives you CSV rows like:

diag_step,S_scaling_slope,S_Lmax,I_box_sep,Chern,diag_tag
0,0.98,3.24,0.57,1,traversal
8,1.01,3.31,0.54,1,traversal
...

	•	S_scaling_slope ~ 1 ⇒ area-law; ~2 ⇒ volume-law (with this min-cut proxy).
	•	I_box_sep drops as correlations fade (nice for phase tracking).
	•	Chern is included iff you pass a states_fn; otherwise it’s omitted.

heck yes — let’s “drop-and-go.” Below are tiny, self-contained modules you can paste in as new files. They don’t refactor your core; they just observe, compute lightweight diagnostics, and log. All optional deps are soft (fail-safe). Defaults keep everything off until you call them.

⸻


# scar_metrics.py
# Minimal scarring/ETH diagnostics + Poincaré sampler (no refactors required)
...
def _safe_norm(z): 
    try: return float(torch.linalg.norm(z).item())
    except: return float(np.linalg.norm(np.asarray(z)))

class ScarDiagnostics:
    """
    Usage:
      scar = ScarDiagnostics(time_op=rcc.time_op, psi=rcc.psi_clock)
      row = scar.tick(dt=0.0)  # returns dict ready to log
    """

    def __init__(self, time_op, psi, device=None):
        self.time_op = time_op
        self.psi0 = psi.detach().clone()
        self.device = device or (self.psi0.device if hasattr(self.psi0, "device") else "cpu")
        self._A = None; self._B = None  # simple default operators

    # --- cheap operators on the clock space
    def _opA(self, dim):
        if self._A is None:
            v = torch.linspace(-1, 1, dim, device=self.device, dtype=self.psi0.dtype)
            self._A = torch.diag(v)
        return self._A

    def _opB(self, dim):
        if self._B is None:
            phase = torch.exp(1j*torch.linspace(0, 2*np.pi, dim, device=self.device, dtype=self.psi0.dtype))
            self._B = torch.diag(phase)
        return self._B

    @torch.no_grad()
    def loschmidt_echo(self, t, eps=1e-3):
        """L(t) = |<ψ| e^{i(H+δ)t} e^{-iHt} |ψ>|^2  (δ≈eps*H)  — lightweight surrogate"""
        H = self.time_op.H_int.to(dtype=torch.complex64)  # diag
        U = torch.matrix_exp(-1j*H*t)
        U_pert = torch.matrix_exp(-1j*(1.0+eps)*H*t)
        amp = (self.psi0.conj() @ (U_pert.conj().T @ (U @ self.psi0)))
        return float(torch.abs(amp)**2)

    @torch.no_grad()
    def otoc_rate(self, t=1.0):
        """Very small OTOC proxy on clock space: C(t)= -< [A(t),B]^2 >; report growth rate."""
        H = self.time_op.H_int.to(dtype=torch.complex64)
        U = torch.matrix_exp(1j*H*t)  # Heisenberg: A(t)=U A U^† with +iHt (since we'll apply on operator)
        dim = H.shape[0]
        A = self._opA(dim); B = self._opB(dim)
        At = U @ A @ U.conj().T
        comm = At @ B - B @ At
        C = -(self.psi0.conj() @ (comm @ (comm @ self.psi0)))
        return float(torch.real(C))

    @torch.no_grad()
    def participation_ratio(self, vec=None):
        """PR = 1 / sum |c_i|^4 — scarred states have anomalously large PR vs nearby energies."""
        v = (vec if vec is not None else self.psi0).flatten()
        p = torch.abs(v)**2 + 1e-12
        return float(1.0 / torch.sum(p**2))

    @torch.no_grad()
    def poincare_sampler(self, param_get, param_set, thetas=(0.0, 0.5, 1.0), t=1.0):
        """
        Tiny loop: nudge a single scalar param and measure return distance.
        param_get/set are callables; returns best theta and min distance.
        """
        try:
            base = float(param_get())
        except Exception:
            return None, None
        H = self.time_op.H_int.to(dtype=torch.complex64)
        U = torch.matrix_exp(-1j*H*t)
        ...
        for th in thetas:
            try:
                param_set(base + th)
                # one-step reversal probe
                Uback = torch.matrix_exp(1j*H*t)
                ret = Uback @ psi_t
                dist = _safe_norm(ret - self.psi0)
                if dist < best[1]: best = (th, dist)
            except Exception:
                continue
        # restore
        try: param_set(base)
    ...
    def tick(self, dt=0.0, **_):
        # use tiny t as “probe time” tied to dt; fall back to 1.0
        t = float(max(1e-3, dt)) if dt else 1.0
        echo = self.loschmidt_echo(t)
        otoc = self.otoc_rate(t)
        pr = self.participation_ratio()
        return {"loschmidt_echo": echo, "otoc_rate": otoc, "scar_overlap": pr}
...
# echo_meter.py
# One-liner wrapper if you want echo as a distinct module

import torch

class EchoMeter:
    def __init__(self, time_op, psi):
        self.time_op = time_op
        self.psi0 = psi.detach().clone()

    @torch.no_grad()
    def measure(self, t: float, eps: float=1e-3) -> float:
        H = self.time_op.H_int.to(dtype=torch.complex64)
        U = torch.matrix_exp(-1j*H*t)
        U_pert = torch.matrix_exp(-1j*(1.0+eps)*H*t)
        amp = (self.psi0.conj() @ (U_pert.conj().T @ (U @ self.psi0)))
...
# lawfield_objective.py
# Make J explicit, expose penalties and a KKT-like residual to logs

import math

def compose_objective(ane_smear: float,
                      penalties: dict | None = None,
                      grads: dict | None = None,
                      lambdas: dict | None = None) -> dict:
    """
    Returns dict with J_total and details; no side effects.
    - penalties: {"qei_violation": max(0, bound-ane), "drift": ...}
    - lambdas:   {"qei": 1.0, "drift": 0.1, ...}
    - grads:     {"g": dJ/dg, "λφ": dJ/dλφ, ...} for residual
    """
    penalties = penalties or {}
    lambdas = lambdas or {}
    pen_sum = 0.0
    for k, v in penalties.items():
        w = float(lambdas.get(k, 0.0))
        pen_sum += w * float(v)

    # KKT-style residual (just a scalar summary)
    if grads:
        gn = math.sqrt(sum(float(g)**2 for g in grads.values()))
    else:
        gn = 0.0

    J = float(ane_smear) + pen_sum
    return {
        "J_total": J,
        "penalty_sum": pen_sum,
        "penalty_vec": {k: float(v) for k, v in penalties.items()},
        "lambda_vec": {k: float(lambdas.get(k, 0.0)) for k in penalties.keys()},
        "kkt_residual": float(gn)
    }
...
# qei_kernel.py
# Drop-in calibration shim — keeps your current bound but logs explicit kernel choice

import numpy as np

_QEI_CACHE = {}

def qei_calibration(kernel: str, sigma_u: float) -> float:
    """
    Returns a dimensionless C_eff so that bound ≈ - C_eff / sigma_u^4.
    This is a placeholder shim (numerical or fitted constants).
    You can plug an exact closed form later without touching callers.
    """
    key = (kernel, round(float(sigma_u), 6))
    if key in _QEI_CACHE: 
        return _QEI_CACHE[key]

    if kernel.lower() == "gaussian":
        # Fitted constant that preserves your current scale; tune later.
        C_eff = 1.0
    elif kernel.lower() in ("fermi", "fermi-dirac", "sigmoid"):
        C_eff = 0.8
    else:
        C_eff = 1.0

    _QEI_CACHE[key] = C_eff
    return C_eff

def qei_bound(kernel: str, sigma_u: float) -> float:
    C = qei_calibration(kernel, sigma_u)
...
# contextuality.py
# KS/contextuality stubs with graceful degradation (no hard deps)

from __future__ import annotations
import math
from typing import Optional

def load_peres_ks_57():
    """
    Returns a minimal mock of the 57-ray KS set (directions on S^2).
    To keep this drop-in, we provide a tiny synthetic subset unless user
    provides a real loader. Replace when you’re ready.
    """
    import numpy as np
    rng = np.random.default_rng(0)
    # unit vectors in R^3 (synthetic stand-ins)
    V = rng.normal(size=(57, 3))
    V /= np.linalg.norm(V, axis=1, keepdims=True) + 1e-12
    return V

def lovasz_theta_lower_bound(adj_matrix) -> Optional[float]:
    """
    Try networkx+cvxpy if available; else return None.
    """
    try:
        import cvxpy as cp
        import numpy as np
    ...
    cons = [cp.diag(X) == 1]
    # Non-edges constraint: X_ij == 0 for edges in complement graph (theta on complement)
    # Here we accept a simple relaxation: set X_ij == 0 on edges to get a lower bound flavor
    for i in range(n):
        for j in range(n):
...
def ks_violation_score(vectors) -> float:
    """
    Super-light heuristic: counts proportion of orthogonal pairs beyond random.
    Real KS proof requires curated sets; this is a placeholder scalar diagnostic.
    """
    import numpy as np
    V = np.asarray(vectors)
    dots = V @ V.T
    ortho = np.sum((np.abs(dots) < 1e-3).astype(float)) - V.shape[0]
    total = V.shape[0] * (V.shape[0] - 1)
    return float(max(0.0, ortho) / max(1.0, total))

def contextual_fraction_stub(viol: float) -> float:
    # Map violation proxy -> [0,1]
    return float(min(1.0, max(0.0, 2.0*viol)))


⸻


# micro_diag_scheduler.py
# One place to run/collect diagnostics and add “heartbeat” flags to each row

from __future__ import annotations
import math
from typing import Optional, Dict

from scar_metrics import ScarDiagnostics
from echo_meter import EchoMeter
from qei_kernel import qei_bound
# contextuality is optional; import lazily inside

def add_heartbeat_fields(row: Dict):
    slope = float(row.get("S_scaling_slope", 2.0))
    row["phase_flag"] = "area" if slope < 1.5 else "volume"
    bound = float(row.get("qei_bound", -1e9))
    ane = float(row.get("ANE_smear", 0.0))
    row["qei_headroom"] = bound - ane
    row["echo_ok"] = bool(row.get("loschmidt_echo", 0.0) > 0.6)

class MicroDiagScheduler:
    """
    Minimal orchestration:
      sched = MicroDiagScheduler(rcc, recorder=rcc.recorder, kernel="gaussian")
      for step in ...:
          row = sched.on_step(dt, context={})
    """
    def __init__(self, rcc, recorder=None, kernel: str="gaussian"):
        self.rcc = rcc
        self.rec = recorder
        self.kernel = kernel
        self.scar = ScarDiagnostics(rcc.time_op, rcc.psi_clock)
        self.echo = EchoMeter(rcc.time_op, rcc.psi_clock)
        self.last_row = None

    def on_step(self, dt: float, context: Optional[Dict]=None) -> Dict:
        sigma_u = float(self.rcc.params.get('sigma_u', 4e-3))
        row = {}
        # QEI bound (explicit kernel)
        row["qei_kernel"] = self.kernel
        row["qei_bound"] = qei_bound(self.kernel, sigma_u)
        # Scar / echo
        row.update(self.scar.tick(dt=dt))
        row["loschmidt_echo_probe"] = self.echo.measure(max(1e-3, dt))
        # Optional: contextuality one-shot (very cheap)
        try:
            from contextuality import load_peres_ks_57, ks_violation_score, contextual_fraction_stub
            rays = load_peres_ks_57()
            viol = ks_violation_score(rays)
            row["ks_violation"] = viol
            row["contextual_fraction"] = contextual_fraction_stub(viol)
        except Exception:
            row["ks_violation"] = None
            row["contextual_fraction"] = None

        add_heartbeat_fields(row)
        self.last_row = row
        if self.rec:
            self.rec.log(row)
        return row
...
How to wire it (no refactor)

Somewhere after you build rcc:

from micro_diag_scheduler import MicroDiagScheduler
from lawfield_objective import compose_objective

sched = MicroDiagScheduler(rcc, recorder=rcc.recorder, kernel="gaussian")

for it in range(num_steps):
    # your existing contractor step
    result = contractor.step(dt, r, do_contract=True)

    # diagnostics pass (cheap)
    row = sched.on_step(dt)

    # OPTIONAL: make J explicit without changing behavior
    penalties = {
        "qei_violation": max(0.0, row["qei_bound"] - result.get("ane_smear", 0.0)),
        "drift": max(0.0, rcc.mera.delta_tensor_energy() - 1e-3)
    }
    lambdas = {"qei_violation": 1.0, "drift": 0.1}
    Jrow = compose_objective(result.get("ane_smear", 0.0), penalties=penalties, lambdas=lambdas)
    if rcc.recorder:
        rcc.recorder.log(Jrow)
